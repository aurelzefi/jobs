                ...
                <span class="relative z-0 inline-flex shadow-sm rounded-md">
                    <template v-for="link in paginator.links">
                        <template v-if="link.url">
                            ...
                            <router-link :to="getPageForLabel(link.label)" v-else v-html="link.label" :aria-label="__('Go to page {label}', {label: link.label})" :class="{ 'rounded-l-md': isThePreviousLink(link), 'rounded-r-md': isTheNextLink(link) }" class="relative inline-flex items-center px-4 py-2 -ml-px text-sm font-medium text-gray-700 bg-white border border-gray-300 leading-5 hover:text-gray-500 focus:z-10 focus:outline-none focus:ring ring-gray-300 focus:border-blue-300 active:bg-gray-100 active:text-gray-700 transition ease-in-out duration-150">
                            </router-link>
                        </template>
